...
game_start = function(){
  ...
  var setPlayer = function(){
    ...
    player.startingX = saveData.hero.x;
    player.startingY = saveData.hero.y;
    ...
    player.name = saveData.hero.name;
    player.characterClass = "Rogue";
    player.exp = saveData.hero.exp;
    player.level = saveData.hero.level;
    player.gp = saveData.hero.gp;
    player.bougu = "";
    player.weapon = "素手";
    ...
    player.hp = saveData.hero.hp;
    player.mp = saveData.hero.mp;
    ...
    player.inventory = saveData.hero.inventory;
  };
  player.displayStatus = function(){
    player.statusLabel.text = 
      ...
      "<br />--GP: " + player.gp +
      "<br />--装備: " + player.bougu +
      "<br />--<button id=\"bougusoubi\">防具を装備</button>" +
      "<br />--武器: " + player.weapon +
      "<br />--<button id=\"weaponsoubi\">武器を装備</button>" +
      "<br /><br />--インベントリ:"; 
    player.statusLabel.height = 240;
    player.showInventory(0);
    const bougusoubi = document.getElementById("bougusoubi");
    bougusoubi.addEventListener("click", () => {
      var bougugamenn = prompt(`${player.inventory}`, player.bougu);
      player.bougu = bougugamenn;
    });
    const bukisoubi = document.getElementById("weaponsoubi");
    bukisoubi.addEventListener("click", () => {
      var weapongamenn = prompt(`${player.inventory}`, player.weapon);
      player.weapon = weapongamenn;
    });
  };
  ...
  // 例：GameDataSave オブジェクト
  GameDataSave = {
    action: function() {
      const JSON_SAVE_DATA = {
        hero: {
         x: player.x, 
         y: player.y, 
      ...
      const blob = new Blob([JSON_SAVE_DATA], { type: "text/json" });
      const url = URL.createObjectURL(blob);

      const a = document.createElement("a");
      a.href = url;
      a.download = "Savedata.DARK_WORLD";  // 自動ダウンロードされるファイル名
      a.click();

      URL.revokeObjectURL(url);
    }
  };
  var spriteRoles = [,,greeter,,cat,,,,,,,,,,,brawler,uragirimono,BossBattle,GameDataSave,]
